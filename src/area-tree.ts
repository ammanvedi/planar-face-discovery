 ...
 * e.g. 4 3 6 7 4
 */
const getPointPath = (nodes: InputNodes, edges: Array<number>): Array<Position> => {
    const pts = edges.slice(0, edges.length - 1);
    return pts.map(p => nodes[p])
}

const getLinePath = (points: Array<Position>): Array<[Position, Position]> => {
    return points.reduce<Array<[Position, Position]>>((
        acc,
        pt,
        ix,
        arr
 ...
 */
export const determineWindingOrder = (points: Array<Position>): WindingOrder => {
 ...
 * Is A inside B
 */
export const isInside = (nodes: InputNodes, p: Position, polygon: Array<Position>): boolean => {
    ...
    do
    {
        ...
        if (intersect(polygon[i], polygon[next], p, extreme))
        {
            ...
            if (determineWindingOrder([polygon[i], p, polygon[next]]) == WindingOrder.COLINEAR)
            {
...
export const getPolygonArea = (nodes: InputNodes, polygon: InputPolygon): number => {
...
const intersect = (p1: Position, q1: Position, p2: Position, q2: Position): boolean => {
...
export const buildNestingTree = (
    nodes: InputNodes,
    polygons: Array<InputPolygon>
): AreaTree => {

    const inputPolygons = polygons.map((p, ix) => ({
        ...p,
        id: ix,
        visited: false,
